...
<!-- Source: tough-tongue-ai/py/jarvis/boxman/claude_box/repo-base/handbooks/ColdCallPatterns.md @ 3ee742b3a (2026-06-14T08:12:34-07:00) -->
...
**When to use:** Creating scenarios where the **AI is the outbound caller** —
calling a lead whose data is (at least partially) known. The user plays the lead.

**Key facts:**

- Voice-pipeline-first: OUTPUT RULES + STT handling + natural speech style mandatory
...
Rules and patterns for creating outbound caller-AI scenarios.

---

## Voice Pipeline Requirements

**CRITICAL**: Every cold call scenario operates in a live voice pipeline (STT → LLM → TTS).
Include ALL THREE blocks in every `ai_instructions`:
...
Always set. Use directive form ("Start with: ..."), not quoted speech. This is the AI's first utterance.
...
### Typical Data Points (5–6 per scenario)

1. Context/size (company size, enrollment, property type)
2. Current solution or situation
3. Biggest pain point / concern
4. Timeline / urgency
5. Budget range or decision authority
6. (Optional) Preferred next step or contact method
...
```
Acknowledge: "I completely understand — [reflect concern]."
Reframe:     "[New angle or low-commitment option]."
Check in:    "[Simple yes/no to re-engage]."
```
...
- "Not interested" said twice
- "Remove me" / "Don't call again"
- Hostile or abusive
- Wrong number / wrong person
- Lead already completed the action
...
user_instructions: |
  ...
  [1–2 sentences of emotional texture: busy, skeptical, stressed, curious, etc.]
  Respond naturally in [language].
  [Optional permissions: "feel free to be price-sensitive", "you may not remember
  filling the form", "you might be in a hurry"]
```
...
**CRITICAL**: Cold call rubriks evaluate the **LEAD** (the human), not the AI.
...
rubrik: |
  ...
  **Recommended action**: [One clear next step]
  ...
  ## Report Format
  **1. Headline** (1 line): Score + outcome + key facts.
  **2. Data** (structured)
  **3. Recommendation**
```
...
```yaml
strategy:
  skip_auto_start: false          # AI speaks first — ALWAYS
  filler_words: "[domain-fit]"
  ...
  conductor:
    ...
    prefix: "CONDUCTOR:"
    messages:
      - time_seconds: [300-450]   # scale to expected call length
        message: "Wrap up — confirm data or next step, end warmly. Use end_session."
        end_turn: true
...
tools_config:
  tools:
    end_session:
      ...
      tool_settings:
        disconnectDelaySeconds: 5  # 8 for sensitive/emotional calls
